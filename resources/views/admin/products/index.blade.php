...
@section('actions')
    <div class="hidden lg:flex space-x-3">
        ...
        <a href="{{ route('admin.products.create.digiflazz') }}"
            ...
            <svg class="w-5 h-5 mr-2"><use href="#icon-refresh"></use></svg>
            Import Digiflazz
        </a>
    </div>
...
@section('content')
    <div class="space-y-6">
        <div class="bg-white dark:bg-slate-800 rounded-[2.5rem] border border-slate-100 dark:border-slate-700 shadow-sm p-6">
            <form method="GET" action="{{ route('admin.products.index') }}" class="space-y-4">
                ...
                <div class="grid grid-cols-2 lg:grid-cols-3 gap-3">
                    <select name="category_id" class="text-xs font-bold rounded-2xl border-none bg-slate-50 dark:bg-slate-900 py-3 focus:ring-2 focus:ring-emerald-500">
                    ...
                    <select name="status" class="text-xs font-bold rounded-2xl border-none bg-slate-50 dark:bg-slate-900 py-3 focus:ring-2 focus:ring-emerald-500">
                        <option value="">Semua Status</option>
                        <option value="active" {{ $status == 'active' ? 'selected' : '' }}>Aktif</option>
                        <option value="inactive" {{ $status == 'inactive' ? 'selected' : '' }}>Nonaktif</option>
                    </select>

                    <button type="submit" class="col-span-2 lg:col-span-1 py-3 rounded-2xl bg-slate-900 dark:bg-emerald-500 text-white font-black uppercase tracking-widest text-[10px] hover:opacity-90 transition-all">
                        Terapkan Filter
                    </button>
                </div>
            </form>
        ...
        @if ($products->count() > 0)
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-4 lg:gap-6">
                @foreach ($products as $product)
                    <div class="flex lg:flex-col">
                        <div class="relative w-28 lg:w-full h-28 lg:h-36 shrink-0 overflow-hidden bg-slate-100 border-r lg:border-r-0 lg:border-b border-slate-50 dark:border-slate-700">
                            @if ($product->image)
                            ...
                            <div class="absolute top-2 left-2 flex flex-col space-y-1">
                                @if($product->is_active)
                                    <span class="px-1.5 py-0.5 bg-emerald-500/90 backdrop-blur-sm text-[7px] font-black text-white uppercase rounded-md shadow-sm">Aktif</span>
                                @else
                                    <span class="px-1.5 py-0.5 bg-rose-500/90 backdrop-blur-sm text-[7px] font-black text-white uppercase rounded-md shadow-sm">Off</span>
                                @endif
                                
                                @if($product->is_featured)
                                    <span class="px-1.5 py-0.5 bg-amber-400/90 backdrop-blur-sm text-[7px] font-black text-slate-900 uppercase rounded-md shadow-sm">Hot</span>
                                @endif
                            </div>
                        </div>
                        ...
                        <div class="p-4 lg:p-5 flex-1 flex flex-col justify-between min-w-0">
                            ...
                            <div class="flex items-center justify-between mt-3 lg:mt-4 pt-3 lg:pt-4 border-t border-slate-50 dark:border-slate-700">
                                <span class="px-3 py-1 bg-slate-100 dark:bg-slate-900 rounded-xl text-[10px] lg:text-xs font-black uppercase tracking-widest text-slate-500">
                                    {{ $product->type ?? ($product->nominals->count() > 1 ? 'Multiple' : 'Single') }}
                                </span>
                                
                                <div class="flex space-x-1">
                                    <a href="{{ route('admin.products.show', $product) }}" class="p-1.5 bg-slate-50 dark:bg-slate-900 rounded-lg text-slate-400 hover:text-emerald-500 transition-colors">
                                        <svg class="w-4 h-4"><use href="#icon-eye"></use></svg>
                                    </a>
                                    <a href="{{ route('admin.products.edit', $product) }}" class="p-1.5 bg-slate-50 dark:bg-slate-900 rounded-lg text-slate-400 hover:text-blue-500 transition-colors">
                                        <svg class="w-4 h-4"><use href="#icon-pencil"></use></svg>
                                    </a>
                                    <button onclick="confirmDelete('{{ $product->id }}')" class="p-1.5 bg-slate-50 dark:bg-slate-900 rounded-lg text-slate-400 hover:text-rose-500 transition-colors">
            ...
            <div class="bg-white dark:bg-slate-800 rounded-[2rem] p-4 border border-slate-100 dark:border-slate-700">
        ...
        @else
            <div class="py-20 text-center bg-white dark:bg-slate-800 rounded-[3rem] border-2 border-dashed border-slate-200 dark:border-slate-700">
                ...
                <a href="{{ route('admin.products.index') }}" class="mt-6 inline-block px-8 py-3 bg-emerald-500 text-white font-black rounded-2xl uppercase text-[10px] tracking-widest shadow-lg">Reset Filter</a>
            </div>
        @endif
    </div>
    ...
    <div id="deleteModal" class="hidden fixed inset-0 z-[60] overflow-y-auto" x-data="{}" x-cloak>
        <div class="flex items-center justify-center min-h-screen p-4">
            ...
            <div class="relative bg-white dark:bg-slate-800 rounded-[2.5rem] w-full max-w-sm p-8 shadow-2xl">
                ...
                <h3 class="text-xl font-black text-center text-slate-800 dark:text-white mb-2 uppercase tracking-tight">Hapus Produk?</h3>
                ...
                <div class="grid grid-cols-2 gap-3">
                    <button onclick="closeDeleteModal()" class="py-4 rounded-2xl bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-white font-bold text-xs uppercase tracking-widest">Batal</button>
                    <form id="deleteForm" method="POST" class="inline">
                        @csrf
                        @method('DELETE')
                        <button type="submit" class="w-full py-4 rounded-2xl bg-rose-500 text-white font-black text-xs uppercase tracking-widest shadow-lg shadow-rose-200 dark:shadow-none">Hapus</button>
                    </form>
                </div>
            </div>
        </div>
